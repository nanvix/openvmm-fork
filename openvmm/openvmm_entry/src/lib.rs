...
fn effective_microvm_filesystem(
    ...
    anyhow::ensure!(
        has_device == restore.microvm_filesystem.is_some()
            && has_device == saved_attachment.is_some(),
    ...
    let Some(saved) = restore.microvm_filesystem.as_ref() else {
        anyhow::ensure!(
            requested.is_none(),
            "a restore-time mount cannot be added to a snapshot without virtio-fs"
        );
        return Ok(None);
    };
...
mod microvm_console_attachment_tests {
    ...
    fn filesystem_contract(root: &Path) -> openvmm_helpers::snapshot::SnapshotMachineContract {
        ...
        .unwrap();
        let (_, attachment) = microvm_filesystem_attachment(root).unwrap();
        let mut command_line = build_microvm_command_line(&[], false).unwrap();
        openvmm_defs::config::append_microvm_virtio_discovery(
            &mut command_line,
            None,
    ...
    #[test]
    fn filesystem_restore_accepts_same_root_at_a_new_path() {
        ...
        assert_eq!(
            restored.root_path,
            fs_err::canonicalize(&moved).unwrap().to_str().unwrap()
        );
    }
...
async fn vm_config_from_command_line(
    ...
    let is_microvm = matches!(
        opt.machine,
        MachineProfileCli::Microvm | MachineProfileCli::MicrovmV2 | MachineProfileCli::MicrovmV3
    );
    let is_microvm_v3 = opt.machine == MachineProfileCli::MicrovmV3;
    ...
    let topology_arch =
        openvmm_defs::config::ArchTopologyConfig::X86(openvmm_defs::config::X86TopologyConfig {
            apic_id_offset: if is_microvm_v3 { 0 } else { opt.apic_id_offset },
            x2apic: if is_microvm_v3 {
    ...
    let mut cfg = Config {
        ...
        processor_topology: ProcessorTopologyConfig {
            ...
            vps_per_socket: if is_microvm_v3 {
                Some(opt.processors)
            } else {
                opt.vps_per_socket
            },
            enable_smt: if is_microvm_v3 {
                Some(false)
            } else {
                match opt.smt {
        ...
        && contract.machine_profile == "microvm"
    {
        let requested_abi_version = match opt.machine {
            ...
            MachineProfileCli::Standard => {
                anyhow::bail!(
                    "microVM snapshot restore requires --machine microvm, microvm-v2, or microvm-v3"
                )
            }
        };
        anyhow::ensure!(

...
### microVM ABI v3 deterministic SMP

`--machine microvm-v3 --processors N` selects the SMP-capable microVM ABI.
...
ABI v3 inherits ABI v2's fixed virtio device slots and sandbox block roles.
Its Xen PVH layout version is 2: the MP table still begins at `0x400`, while
...
ABI v1 and v2 retain their original one-vCPU PVH layout version 1.
...
v1/v2 snapshots remain one-vCPU compatible and are not accepted as v3
baselines. For example:

```shell
openvmm --machine microvm-v3 --processors 8 \
